...
// timesMs mirrors how useReplayStore.loadWindow derives sampleTimesMs from
// samples, so these tests exercise the same samples/sampleTimesMs pairing
// computeFlightsAtTime sees in production.
function timesMs(samples: ReplaySample[]): number[] {
  return samples.map((s) => new Date(s.recorded_at).getTime());
}

describe("computeFlightsAtTime", () => {
  it("returns an empty list when atMs is null", () => {
    const samples = [sample()];
    expect(computeFlightsAtTime(samples, timesMs(samples), null)).toEqual([]);
  });
  ...
  it("reconstructs each aircraft's most recent sample at or before atMs", () => {
    ...
    const samples: ReplaySample[] = [
      ...
      sample({
        icao24: "b",
        recorded_at: new Date(1500).toISOString(),
        lat: 9,
      }),
      sample({
        icao24: "a",
        recorded_at: new Date(2000).toISOString(),
        lat: 2,
      }),
    ];

    const flights = computeFlightsAtTime(samples, timesMs(samples), 1800);
  ...
  it("excludes aircraft whose only samples are after atMs", () => {
    ...
    expect(computeFlightsAtTime(samples, timesMs(samples), 1000)).toEqual([]);
  });
  ...
  it("includes a sample exactly at atMs", () => {
    ...
    expect(computeFlightsAtTime(samples, timesMs(samples), 2000)).toHaveLength(
      1,
    );
  });
  ...
  it("fills in canonical FlightState fields the sample doesn't carry", () => {
    ...
    const [flight] = computeFlightsAtTime(samples, timesMs(samples), 1000);
