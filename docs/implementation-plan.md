...
3. **One source adapter end-to-end** (M) — pick the simplest provider first (airplanes.live: no auth required) and get raw payload → `ingest.raw.airplaneslive` style publish working against Redis directly (NATS isn't introduced until Phase 2, so this writes straight through for now) plus fixture-based contract tests. This proves the adapter pattern before replicating it twice more.
