...
## CI quality gates (planned operations checks, enforced before code ships)
...
All of the above are intended to run on every PR via GitHub Actions (see [`hosting-and-deployment.md`](hosting-and-deployment.md) for the intended pipeline including deploy).
