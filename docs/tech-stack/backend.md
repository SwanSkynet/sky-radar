...
- The public REST surface is versioned at `/api/v1` and its OpenAPI schema is published at `GET /api/v1/openapi.yaml`, generated from the single source of truth at `cmd/apigateway/openapi-v1.yaml` (embedded into the binary) and exercised by contract tests in `cmd/apigateway/contract_test.go`. See [`../architecture/data-model.md`](../architecture/data-model.md)'s "API authentication & rate limiting" section for the tier/auth model. GraphQL schema publication is deferred to a follow-up milestone — no GraphQL layer exists yet.
