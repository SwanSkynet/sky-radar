...
/docs
```
Each `/cmd` entry is an independently deployable binary/container; they share code only through `/internal`, never through direct imports of each other — this is what makes the bulkhead isolation described in [`system-architecture.md`](../architecture/system-architecture.md) actually hold.
