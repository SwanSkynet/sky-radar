...
- `docker-compose.yml` at the repo root defines the full local-dev stack (every service + NATS + Redis + Postgres + Grafana/Prometheus, per [`observability-and-ops.md`](observability-and-ops.md)) — this is also the reference topology for anyone self-hosting.
