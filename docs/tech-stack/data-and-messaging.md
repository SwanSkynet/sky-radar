...
**Partitioning:** `flight_history` is time-partitioned (by day or week) so old partitions can be dropped or downsampled-further on a retention schedule without expensive `DELETE` queries — necessary for keeping storage cost bounded over time on a free/cheap-tier database. The initial migration creates only a `DEFAULT` partition to stay self-contained; rolling per-day/week range partitions is Phase 3 ops automation. **Rollover note for that automation:** Postgres refuses to `ATTACH` a new range partition whose bounds overlap rows still sitting in the `DEFAULT` partition, so the rollover job must first move (or delete) any `flight_history_default` rows that fall in the incoming partition's range before attaching it, or the automation will fail outright instead of degrading gracefully.
