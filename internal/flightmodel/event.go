...
// NewEventID returns a randomly generated UUID (v4, RFC 4122) for use as
// Event.ID. Rule implementations call this when constructing an Event
// rather than each maintaining their own ID generation.
func NewEventID() string {
	var b [16]byte
	if _, err := rand.Read(b[:]); err != nil {
		panic(fmt.Sprintf("flightmodel: read random bytes for event id: %v", err))
	}
